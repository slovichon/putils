...
void
pfile(int j, struct file *fil, char *fileflags)
{
	int i, saw, pr_rdwr;
	struct vnode vn;
	const char *p = "???";

	if (fil->f_type == DTYPE_VNODE) {
		if (kvm_read(kd, (u_long)fil->f_data, &vn,
		    sizeof(vn)) != sizeof(vn))
			err(EX_OSERR, "kvm_read: %s", kvm_geterr(kd));
		switch (vn.v_type) {
		case VFIFO:	p = "S_IFIFO";  break;
		case VCHR:	p = "S_IFCHR";  break;
		case VDIR:	p = "S_IFDIR";  break;
		case VBLK:	p = "S_IFBLK";  break;
		case VREG:	p = "S_IFREG";  break;
		case VLNK:	p = "S_IFLNK";  break;
		case VSOCK:	p = "S_IFSOCK"; break;
	//	case VWHT:	p = "S_IFWHT";  break;
		}
	}
